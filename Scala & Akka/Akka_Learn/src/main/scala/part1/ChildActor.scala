...
object ChildActor extends App {
  ...
  class EmployeeActor extends Actor {
    override def receive: Receive = {
      case task: Task => println(s"${self.path} , i got task : ${task.message.toString}")
    }
  }


  case class CreateEmployee(name: String)
  ...
  class ManagerActor extends Actor {
    ...
    override def receive: Receive = {
      case CreateEmployee(name) => {
        ...
        val childRef = context.actorOf(Props[EmployeeActor], name)
        context.become(withEmployee(childRef))
      }
    }

    def withEmployee(childRef: ActorRef): Receive = {
      case task: Task => childRef forward task
    }

  }


  val parent = system.actorOf(Props[ManagerActor], "manager")
  //  parent ! CreateEmployee("employee1")
  //  parent ! Task(s"Task1 ")
  ...
  val childSelection = system.actorSelection("/user/manager/employee1")

  //  childSelection ! Task("Another Task")
